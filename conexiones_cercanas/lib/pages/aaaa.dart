...
class HomePageState extends State<HomePage> {
  ...
  List<serial.BluetoothDevice> _devicesList = [];
  FlutterLocalNotificationsPlugin flutterLocalNotificationsPlugin;
  serial.FlutterBluetoothSerial _bluetooth = serial.FlutterBluetoothSerial.instance;
  ...
  if(this.mounted && devicesReady == true){
    setState(() {
  ...
  if(devicesReady == true){

    if (_devicesList.length > 0){
    for(serial.BluetoothDevice a in _devicesList ){
    for (ScanResult r in results) {

      /*if(r.advertisementData.connectable == true){
        r.device.name = r.device.id.toString() + ' (Bonded)';
      }*/

      if(a.address.toString() == r.device.id.toString()){
        r.device.name = a.name.toString() + ' (Bonded)';
      }
        if(r.rssi >=-69 && alarma.sonando == false && (a.address.toString() == r.device.id.toString()) ){ 
          if(isSwitched == false){
            alarma.sonar();
          }else{
